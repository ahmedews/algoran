...
""",
        ...
        if len(m.command) < 2:
            ...
            if search == 0:
                await huehue.edit("لم يتم العثور على شيء , اعطني اسم القاري كاملℹ️")
            else:
                songname = search[0]
                url = search[1]
...
""",
        ...
        if len(m.command) < 2:
            ...
            hmmm = HighQualityVideo()
            if search == 0:
                await huehue.edit("**لم يتم العثور على شيء , اعطني اسم القاري كامل**")
            else:
                songname = search[0]
                url = search[1]
                hm, ytlink = await ytdl(url)
...
@Client.on_message(filters.command(["قران"], prefixes=f"{HNDLR}"))
async def playfrom(client, m: Message):
    chat_id = m.chat.id
    if len(m.command) < 2:
        await m.reply(
            f"**الاستخدام:** \n\n`{HNDLR}قران [بالايدي/بالمعرف]` \n`{HNDLR}قران [بالايدي/بالمعرف]`"
        )
    else:
